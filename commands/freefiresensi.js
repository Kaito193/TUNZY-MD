...
async function freefireSensitivityCommand(sock, chatId, message, userMessage) {
    ...
    if (userStates.has(senderId)) {
        ...
        if (state.step === 2) {
            ...
            if (parts.length < 2) {
                await sock.sendMessage(chatId, {
                    text: '❌ Invalid format. Please use: *RAM/Space, Android/iPhone*\nExample: *8GB/128GB, Android*'
                }, { quoted: message });
                return;
            }
            ...
            if (!isAndroid && !isIOS) {
                await sock.sendMessage(chatId, {
                    text: '❌ Please specify either *Android* or *iPhone/iOS*'
                }, { quoted: message });
                return;
            }
            ...
            // Create caption
            let caption = `🎮 *${sensitivityData.deviceName}* 📛\n\n` +
            ...
            if (sensitivityData.dpi) {
                ...
                caption += `\n📱 *iPhone/iOS - No DPI Settings*\n\n`;
            }
            
            caption += `📊 *Device:* ${ramSpace}\n` +
                      `🤖 *Platform:* ${deviceType}\n\n` +
                      `Generated by TUNZY-MD 🫶`;
    ...
    if (userMessage === '.freefireesensi') {
        ...
        await sock.sendMessage(chatId, {
            text: '🎮 *FREE FIRE SENSITIVITY GENERATOR*\n\n' +
                  '*Step 1/3*\n\n' +
...
async function createSensitivityImage(data) {
    try {
        ...
        ctx.fillText('TUNZY-MD 🫶', image.width - 250, image.height - 50);
...
async function createFallbackImage(data) {
    ...
    ctx.fillText('TUNZY - MD - SENSITIVITY 🫶', width / 2, 100);
    ...
    ctx.font = '32px Arial';
    ctx.textAlign = 'left';
    ctx.fillText(`📱 Device: ${data.ramSpace}`, 150, height - 150);
    ctx.fillText(`🤖 Platform: ${data.deviceType}`, 150, height - 100);
